...
from replay_buffer import ReplayBufferStorage, make_replay_loader, make_expert_obsaction_replay_loader
...
class Workspace:
    ...
    def train_bc(self):
        # predicates
        train_until_step = utils.Until(self.cfg.stage1_train_frames, self.cfg.suite.action_repeat)
        eval_every_step = utils.Every(self.cfg.suite.eval_every_frames, self.cfg.suite.action_repeat)
        ...
        while train_until_step(self.global_step):
            ...
            # Update
            metrics = self.agent.bc_update(self.expert_replay_iter, self.global_step)
    ...
    def train_il(self):
        ...
        seed_until_step = utils.Until(self.cfg.suite.num_seed_frames + self.cfg.stage1_train_frames,
                                      self.cfg.suite.action_repeat)
        ...
        if self.agent.auto_rew_scale:
            self.agent.sinkhorn_rew_scale = 1.
            self.start_episode = self._global_episode    # Set after first episode
        ...
        while train_until_step(self.global_step):
            if time_step.last():
                ...
                if self.agent.auto_rew_scale:
                    if self._global_episode == (self.start_episode + 1):
...
def main(cfg):
    from train_vrf import Workspace as W
    root_dir = Path.cwd()
    ...
    # copy weights to encoder_bc and actor_bc
    workspace.agent.load_stage1()

    # ROT training
    episode_reward, success_percentage = workspace.train_il()
