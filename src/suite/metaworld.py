...
CAMERA = {
	...
	'window-close-v2': 'corner',
	'lever-pull-v2': 'corner',
...
class RGBArrayAsObservationWrapper(dm_env.Environment):
	...
	def get_frame(self):
		frame = self._env.render(offscreen=True, camera_name=self.camera_name)  # 500 * 300
...
def make(name, frame_stack, action_repeat, seed):
	...
	if name in ['coffee-pull-v2']:
		ml1 = metaworld.MT1(name)
	else:
	...
	img_size = 224
	# add wrappers
	env = RGBArrayAsObservationWrapper(env, ml1,
